<div class="row mb-5">
    <div class="col-6 ">
        <div class="img-content"></div>
    </div>
    <div class="row col-6" style="margin-left:10px;">
        <div class="col-12 mt-5"> 
            <p class="h1"><b>Faça seu orçamento de forma simples com nosso designer.</b></p>
            <p class="h5">Ao se cadastrar, ficarar por dentro de todas as possiveis atividades e produtos disponiveis que a empresa entrega de acordo com a necessidade, tornando possivel contruir sites e sistemas com um código <b>impecável</b>.</p>
        </div>
    </div>
</div>
<div class="row" style="margin-top: 17px;margin-bottom:130px;">
    <div class="row col-6">
        <div class="col" style="margin-top:190px;"> 
            <p class="h1"><b>Tecnologia e segurança garantida.</b></p>
            <p class="h5 mt-4">Nossa prioridade é garantir a segurança absoluta dos nossos sites e sistemas. Investimos em medidas robustas de proteção cibernética, implementando as mais recentes tecnologias de segurança e adotando práticas de desenvolvimento seguro. </p>
        </div>
    </div>
    <div class="col-6 ">
        <div class="img-security"></div>
    </div>
</div>